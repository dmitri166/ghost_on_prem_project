#!/bin/bash
# Ghost Platform - Self-Hosted GitHub Runner Setup Script (Auto Token)
# This script automatically gets runner registration token
...
echo -e "${GREEN}🚀 Setting up Ghost Platform GitHub Runner (Auto Token)...${NC}"
...
# Get runner registration token automatically
echo -e "${YELLOW}🔑 Getting runner registration token...${NC}"
RUNNER_TOKEN=$(curl -s -X POST \
  -H "Authorization: token $GITHUB_TOKEN" \
  -H "Accept: application/vnd.github.v3+json" \
  https://api.github.com/repos/dmitri166/ghost_on_prem_project/actions/runners/registration-token | \
  jq -r .token)
...
echo -e "${GREEN}✅ Runner registration token obtained${NC}"
...
# Configure runner with auto token
echo -e "${YELLOW}⚙️ Configuring runner...${NC}"
echo "$RUNNER_TOKEN" | ./config.sh \
    --url "$REPO_URL" \
    --token "$RUNNER_TOKEN" \
    --name "$RUNNER_NAME" \
